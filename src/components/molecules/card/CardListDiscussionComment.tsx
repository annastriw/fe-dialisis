...
export default function CardListDiscussionComment({ data, isLoading }: CardListDiscussionCommentProps) {
  if (isLoading) {
    return (
      <div className="flex flex-col gap-6">
        {[...Array(3)].map((_, i) => (
          <div className="flex flex-col sm:flex-row w-full gap-3 space-y-4 sm:space-y-0" key={i}>
  ...
  if (data.length === 0) {
    return (
      <div className="text-muted-foreground flex flex-col items-center justify-center gap-2 rounded-xl border border-dashed p-6 text-center">
        ...
        <p className="text-sm">Belum ada obrolan diskusi nih! Mulai diskusi yuk ✨</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      {data.map(comment => (
        <div key={comment.id} className="flex flex-col sm:flex-row w-full gap-3 space-y-2 sm:space-y-0">
          ...
          <div className="flex-1 space-y-2">
            <Card className="bg-muted w-full border-0 p-2 shadow-none">
              <CardContent className="px-3 py-2 space-y-2">
                ...
                {comment.image_path && (
                  <Image src={`${BASE_URL}/public/storage/${comment.image_path}`} alt="Foto" width={1000} height={1000} className="rounded-xl max-w-full h-auto md:max-w-sm" />
                )}
                <p>{comment.comment}</p>
              </CardContent>
            </Card>
            <p className="text-muted-foreground text-sm">{formatRelativeTime(comment.created_at)}</p>
            <Link href={`/dashboard/discussions/${comment.id}/answers`} className="text-muted-foreground hover:underline text-sm">
