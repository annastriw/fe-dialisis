// src/components/molecules/card/CardListReportHistoryScreeningMedical.tsx
...
interface CardListReportHistoryScreeningMedicalProps {
  data: Screening[];
  isLoading: boolean;
}

export default function CardListReportHistoryScreeningMedical({
  data,
  isLoading,
}: CardListReportHistoryScreeningMedicalProps) {
  if (isLoading) {
    return (
  ...
  return (
    <div className="space-y-4">
      {data.map((screening) => (
        <Link
          href={`/dashboard/medical/reports/history/screening/${screening.id}`}
          key={screening.id}
