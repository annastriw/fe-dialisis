...
import {
  getLatestScreeningASBHDAVFHandler,
  ScreeningASBHDAVFLatest,
} from "@/http/screening-asbhd-avf/get-latest-screening-asbhd-avf";
import { Badge } from "@/components/ui/badge";
import { Card, CardHeader, CardTitle } from "@/components/ui/card";
import { Skeleton } from "@/components/ui/skeleton";
import { FileSearch, Check } from "lucide-react";
import DialogStartScreeningASBHDAVF from "@/components/atoms/dialog/DialogStartScreeningASBHDAVF";

export default function CardListScreeningASBHDAVF() {
  const { data: session, status } = useSession();
  const [latest, setLatest] = useState<ScreeningASBHDAVFLatest | null>(null);
  ...
  useEffect(() => {
    ...
    getLatestScreeningASBHDAVFHandler(session.access_token)
      .then((res) => {
        console.log("✅ Latest ASBHD-AVF response:", res);
        setLatest(res);
      })
      .catch((err) => console.error("❌ Error fetching latest ASBHD-AVF", err))
            ...
            <CardHeader className="flex md:flex-row md:items-center md:justify-between">
              <div className="space-y-2">
                ...
                <CardTitle className="text-md font-bold md:text-xl">
                  Assessment Self-Care Behaviours with Arteriovenous Fistula
                  (ASBHD-AVF)
                </CardTitle>
