...
interface CardListHistoryScreeningCKDSCProps {
  data: ScreeningCKDSCHistoryItem[];
  isLoading: boolean;
}

export default function CardListHistoryScreeningCKDSC({
  data,
  isLoading,
}: CardListHistoryScreeningCKDSCProps) {
  if (isLoading) {
    return (
    ...
    return (
      <div className="flex flex-col items-center justify-center space-y-4 py-10 text-center">
        ...
        <p className="text-muted-foreground">
          Belum ada riwayat screening CKDSC.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {data.map((item) => {
        ...
        return (
          <Link
            href={`/dashboard/history/screening-ckdsc/${item.id}`}
            key={item.id}
