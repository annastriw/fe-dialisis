...
import {
  getLatestScreeningCKDSCHandler,
  ScreeningCKDSCLatest,
} from "@/http/screening-ckdsc/get-latest-screening-ckdsc";
import { Badge } from "@/components/ui/badge";
import { Card, CardHeader, CardTitle } from "@/components/ui/card";
import { Skeleton } from "@/components/ui/skeleton";
import { FileSearch, Check } from "lucide-react";
import DialogStartScreeningCKDSC from "@/components/atoms/dialog/DialogStartScreeningCKDSC";

export default function CardListScreeningCKDSC() {
  const { data: session, status } = useSession();
  const [latest, setLatest] = useState<ScreeningCKDSCLatest | null>(null);
  ...
  useEffect(() => {
    ...
    getLatestScreeningCKDSCHandler(session.access_token)
      .then((res) => {
        console.log("✅ Latest CKDSC response:", res);
        setLatest(res);
      })
      .catch((err) => console.error("❌ Error fetching latest CKDSC", err))
            ...
            <CardHeader className="flex md:flex-row md:items-center md:justify-between">
              <div className="space-y-2">
                ...
                <CardTitle className="text-md font-bold md:text-xl">
                  Chronic Kidney Disease Self-Care (CKDSC)
                </CardTitle>
